...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        {
            return users.stream().sorted(Comparator.comparingInt(User::getScore)).collect(Collectors.toList());
        }
        else if(scoreOrder == scoreOrder.DESC)
        {
            return users.stream().sorted(Comparator.comparingInt(User::getScore).reversed()).collect(Collectors.toList());
        }
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {
        Contest contest = contestRepository.findById(contestId).orElseThrow(() -> new ContestNotFoundException("ContestNotFoundException"));
        User user = userRepository.findByName(userName).orElseThrow(() -> new UserNotFoundException("UserNotFoundException"));
        if(!user.checkIfContestExists(contest))
        {
